...
//Measure nuclear BrdU intensity
image = getTitle();
roiManager("reset");
run("Clear Results");
run("Split Channels");
...
run("Convert to Mask");
run("Watershed");
run("Analyze Particles...", "size=50-500 display exclude clear add");
selectWindow("C2-"+image);
run("Set Measurements...", "area integrated redirect=None decimal=3");
run("Clear Results");
roiManager("deselect");
roiManager("Measure");
waitForUser("Please copy data from the result table for BrdU intensity");
